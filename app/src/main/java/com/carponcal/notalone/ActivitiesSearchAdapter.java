...
/**
 * Adaptador para el recycler del buscador de actividades
 */
public class ActivitiesSearchAdapter extends   RecyclerView.Adapter <ActivitiesSearchAdapter.ListaViewHolder> {
    protected List<Activity_user> activities;
    protected LayoutInflater inflador;   //Crea Layouts a partir del XML
    protected Context contexto;          //Lo necesitamos para el inflador
    protected View.OnClickListener onClickListener;

    public ActivitiesSearchAdapter(Context context, List<Activity_user>  objects) {
    ...
    @Override public void onBindViewHolder(ListaViewHolder viewHolder, int i) {
        ...
        if (diferenciaTiempo < 0){
            ...
            if (dias > 0) {
                viewHolder.planification.setText("Quedan " + dias + " días y " + horas + ":" + ((minutos<10)?'0'+minutos:minutos) + " horas");
            } else {
                if (horas > 0) {
                    viewHolder.planification.setText("Quedan solo : " + horas + ":" + ((minutos<10)?'0'+minutos:minutos) + " horas");
                } else {
